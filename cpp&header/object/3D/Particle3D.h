...
public:
	...
	Particle3D(int shaderNum, const string& filename);
	~Particle3D();
...
template<typename T>
HRESULT Particle3D::CreateConstBuffer(ComPtr<ID3D12Resource>& buffer)
{
	HRESULT result;
...
template<typename T>
void Particle3D::UpdateBuffer(ComPtr<ID3D12Resource>& buffer, T& bufferData)
{
	T* map = nullptr;
	HRESULT result;
