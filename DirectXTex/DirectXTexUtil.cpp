...
bool DirectX::_DXGIToWIC(DXGI_FORMAT format, GUID& guid, bool ignoreRGBvsBGR) noexcept
{
    switch (format)
    {
        ...
        case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
            if (ignoreRGBvsBGR)
            {
                // If we are not doing conversion so don't really care about BGR vs RGB clearColor-order,
...
//-------------------------------------------------------------------------------------
// Returns bits-per-clearColor-channel for a given DXGI format, or 0 on failure
// For mixed formats, it returns the largest clearColor-depth in the format
//-------------------------------------------------------------------------------------
_Use_decl_annotations_
